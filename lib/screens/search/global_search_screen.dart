...
class _GlobalSearchScreenState extends State<GlobalSearchScreen> {
    ...
    return Column(
      children: [
        Padding(
          ...
          child: SearchBar(
            hintText: 'Search inventory, suppliers, purchases, menu, recipes, sales, users',
                            ...
                            onTap: result.onTap ??
                                () {
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    SnackBar(
                                      content: Text('Open ${result.module}: ${result.title}'),
                                    ),
                                  );
                                },
                          ),
  ...
  List<SearchResultModel> _buildResults(BuildContext context, String query) {
    ...
    for (final item in inventoryProvider.items) {
      if (matches(item.itemName) || matches(item.category)) {
        results.add(SearchResultModel(
          id: item.id,
          title: item.itemName,
          subtitle: '${item.category}, ${Formatters.quantityWithUnit(item.totalStock, item.unit)}',
          module: 'Inventory',
    ...
    for (final supplier in supplierProvider.suppliers) {
      if (matches(supplier.name) || matches(supplier.email) || matches(supplier.phone)) {
        results.add(SearchResultModel(
          id: supplier.id,
          title: supplier.name,
    ...
    for (final purchase in purchaseProvider.purchases) {
      if (matches(purchase.itemName) ||
        ...
        results.add(SearchResultModel(
          ...
          subtitle: '${purchase.supplierName}, ${Formatters.currency(purchase.totalAmount)}',
          module: 'Purchases',
    ...
    for (final item in kitchenProvider.menuItems) {
      if (matches(item.name) || matches(item.section)) {
        results.add(SearchResultModel(
          id: item.id,
          title: item.name,
          subtitle: '${item.section}, ${Formatters.currency(item.sellingPrice)}',
          module: 'Menu',
    ...
    for (final recipe in kitchenProvider.recipes) {
      if (matches(recipe.menuItemName) ||
          recipe.ingredients.any((ingredient) => matches(ingredient.ingredientName))) {
        results.add(SearchResultModel(
          id: recipe.id,
    ...
    for (final food in kitchenProvider.preparedFood) {
      if (matches(food.menuItemName)) {
        results.add(SearchResultModel(
          ...
          subtitle: '${Formatters.number(food.quantityAvailable)} prepared units',
    ...
    for (final sale in kitchenProvider.sales) {
      if (matches(sale.menuItemName) || matches(sale.paymentMethod)) {
        results.add(SearchResultModel(
          ...
          subtitle: '${sale.paymentMethod}, ${Formatters.currency(sale.totalAmount)}',
          module: 'Sales',
    ...
    for (final movement in kitchenProvider.stockMovements) {
      if (matches(movement.itemName) ||
        ...
        results.add(SearchResultModel(
          ...
          subtitle: '${movement.action}, ${Formatters.quantityWithUnit(movement.quantity, movement.unit)}',
    ...
    for (final user in adminProvider.users) {
      if (matches(user.email) || matches(user.displayName ?? '') || matches(user.role)) {
        results.add(SearchResultModel(
          id: user.uid,
